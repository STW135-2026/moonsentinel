...
保存政策名称、允许用途、允许接收方、同意字段、列分类与处置、最小匿名组大小及诊断上限。
构造时执行以下失败关闭约束：
...
`PrivacyReport` 保存总拒绝数、受限 findings、approved/quarantine 行索引、发布列和 k 值。
...
- `manifest`：政策、请求、用途、接收方、决策、行数、k 和发布列。
...
6. 隔离组大小小于 k 的候选行；
7. 按源顺序生成 approved 与 quarantine；
8. 对 approved 执行显式列投影和替换；
9. 生成 findings 和单行 release manifest。
